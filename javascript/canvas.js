...
function adjacentMatches (matchSnood, existingMatches) {
  ...
  var matches = adjacentSnoods(matchSnood).filter((snood) => {
  ...
  newMatches.forEach ((snood) => {
    newMatches = newMatches.concat(
      adjacentMatches(snood, allMatches)
    );
  });
...
function adjacentSnoods(snood) {
  ...
  var snoods = adjacentPositions.map((pos) => {
    if (enemies[pos.col] &&
  ...
  return snoods.filter((snood) => {
    if (snood) {return snood;}
  });
}
...
function collisionDetection () {
  ...
  for(var col = 0; col < enemies.length; col++) {
    for(var row = 0; row < enemyRowCount; row++) {
      ...
      if(target.alive) {
        gameWon = false;
        if(snoodX + snoodSize > target.x - snoodSize &&
          snoodX - snoodSize < target.x + snoodSize &&
          snoodY + snoodSize > target.y - snoodSize &&
          snoodY - snoodSize < target.y + snoodSize) {
  ...
  enemies.forEach((row) => {
    row.forEach((snood) => {
      if (!checked.includes(snood)) {
        ...
        var anchored = cluster.some((clusterSnood) => {
          return clusterSnood.col === 0;
        });

        if (!anchored) {
          cluster.forEach((snood) => snood.color = "#000000");
        }
...
function getCluster(snood, included) {
  ...
  if (newSnoods.length === 0) { return included; }
  included = included.concat(newSnoods);

  var allSnoods = [];
  newSnoods.forEach((newSnood) => {
